...
import { Check, X, Warning, PencilSimple, Trash, TelegramLogo, Robot, Camera } from "@phosphor-icons/react";
import { useApp } from "../../data/store";
import { formatIDR, terbilang } from "../../lib/format";
import { fmtFullDateID, todayISO } from "../../lib/dates";
import { AmountInput, Badge, Button, Card, ConfirmDialog, EmptyState, Field, Input, Pagination, Select, Sheet, usePagination, useToast } from "../../components/ui";
...
export function ApprovalsPage() {
  const { data, approveDraft, rejectDraft, deleteDraft } = useApp();
  ...
  return (
    <div>
              ...
              <div key={d.id} className="flex items-center gap-3 px-4 py-3.5">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-ink">{d.merchant}</span>
                  ...
                  <span className="tnum shrink-0 text-sm font-semibold text-ink">{formatIDR(d.amount)}</span>
                </button>
                <button
                  onClick={() => setConfirmDelId(d.id)}
                  aria-label={`Hapus draft ${d.merchant}`}
                  className="shrink-0 rounded-lg p-1.5 text-ink-muted hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950"
                >
      ...
      <ConfirmDialog
        ...
        title="Hapus draft?"
        body="Draft ini akan dihapus permanen beserta lampiran struknya."
        confirmLabel="Hapus"
        onConfirm={() => {
          if (confirmDelId) deleteDraft(confirmDelId);
          setConfirmDelId(null);
          toast.push("success", "Draft dihapus");
        }}
    ...
    <Sheet
      ...
      footer={
        editing ? (
          ...
          <div className="flex gap-3">
            <Button variant="danger" className="flex-1" onClick={() => onReject(draft.id)}>
              <X size={16} weight="bold" /> Tolak
            </Button>
            <Button className="flex-1" onClick={() => onApprove(draft.id, {})}>
    ...
    >
      <div className="space-y-4">
        <div className="rounded-xl bg-canvas p-3">
          <p className="tnum text-2xl font-bold tracking-tight text-ink">{formatIDR(draft.amount)}</p>
        ...
        {editing ? (
          ...
          <>
            <p className="text-sm font-semibold text-ink">{draft.merchant}</p>
            <p className="whitespace-pre-line -mt-2 text-xs text-ink-muted">{draft.description}</p>
            ...
            <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 text-sm font-semibold text-brand-700 dark:text-brand-300">
              <PencilSimple size={14} weight="bold" /> Edit dulu sebelum setuju
            </button>
          </>
        )}
      </div>
